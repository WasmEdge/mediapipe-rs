...
const MODEL_ASSET: &str = "assets/models/gesture_recognition/gesture_recognizer.task";

const THUMBS_UP_IMG: &str = "assets/testdata/img/thumbs_up.jpg";
const THUMBS_DOWN_IMG: &str = "assets/testdata/img/thumbs_down.jpg";
const VICTORY_IMG: &str = "assets/testdata/img/victory.jpg";
const POINTING_UP_IMG: &str = "assets/testdata/img/pointing_up.jpg";
...
fn test_gesture_recognition() {
    ...
    let gestures = &thumbs_up_res.first().unwrap().gestures;
    eprintln!("{}", gestures);
    ...
    let gestures = &thumbs_down_res.first().unwrap().gestures;
    eprintln!("{}", gestures);
    ...
    let gestures = &victory_res.first().unwrap().gestures;
    eprintln!("{}", gestures);
    ...
    let gestures = &pointing_up_res.first().unwrap().gestures;
    eprintln!("{}", gestures);
...
fn draw_hand_landmarks(
    ...
    mediapipe_rs::postprocess::utils::draw_landmarks_with_options(
        &mut img,
        &res.first().unwrap().hand_landmark.hand_landmarks,
        &options,
    );
    img.save(out).unwrap();
}
